...
for(const width of [1440,768,390,360]){
 ...
 for(const route of ['/','/services/domestic','/services/international']){
  ...
  if(route==='/')await page.locator('.process-flow').screenshot({path:`artifacts/same-photo-repair/${phase}-four-steps-${width}.png`,style:'header{visibility:hidden}',scale:'css'});
  else {await page.evaluate(()=>scrollTo({top:0,behavior:'instant'}));await page.screenshot({path:`artifacts/same-photo-repair/${phase}-${route.split('/').pop()}-${width}.png`,scale:'css'});}
...
await writeFile(`artifacts/same-photo-repair/${phase}.json`,JSON.stringify(results,null,2));await browser.close();console.log(`${phase}: ${results.length} layouts captured`);
