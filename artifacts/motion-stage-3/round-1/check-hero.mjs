import {createRequire} from 'node:module'; const require=createRequire('/Users/pranshudhiman/Desktop/vk and company/package.json'); const {chromium}=require('@playwright/test');import {readFile,writeFile} from 'node:fs/promises';import assert from 'node:assert/strict';
const browser=await chromium.launch();const dir='/Users/pranshudhiman/Desktop/vk and company/artifacts/motion-stage-3/round-1';const results=[];const before=JSON.parse(await readFile('/Users/pranshudhiman/Desktop/vk and company/artifacts/motion-stage-3/before/layout.json','utf8'));
for(const scenario of [{name:'desktop',width:1440},{name:'mobile',width:390,touch:true},{name:'desktop-reduced',width:1440,reduced:true},{name:'mobile-reduced',width:390,touch:true,reduced:true},{name:'desktop-no-js',width:1440,nojs:true},{name:'mobile-no-js',width:390,nojs:true}]){
 ...
 if(record){assert.equal(initial.entries.length,4);assert.deepEqual(initial.entries.map(e=>e.options.duration),[600,600,400,400]);assert.deepEqual(initial.entries.map(e=>e.options.delay),[0,170,770,860]);assert(initial.entries.every(e=>JSON.stringify(e.options.easing.match(/[\d.]+/g).map(Number))==='[0.16,1,0.3,1]'));assert(Math.max(...initial.entries.map(e=>e.startTime))-Math.min(...initial.entries.map(e=>e.startTime))<1);}
 ...
 assert(pointer.every(p=>Math.abs(p.x)<=6&&Math.abs(p.y)<=6));
