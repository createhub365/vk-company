...
test("same ordered list, four decoded equal photo frames, horizontal desktop and vertical mobile", async ({ page }) => {
  ...
  await page.screenshot({ path: `artifacts/motion-stage-9/regression/screenshots/process-${page.viewportSize()!.width}.png`, scale: "css" });
});
...
test("tall viewport settles on a stable middle blend and survives resize", async ({ page }) => {
  ...
  await page.screenshot({ path: `artifacts/motion-stage-9/regression/screenshots/tall-${test.info().project.name}.png`, scale: "css" });
...
test("reduced motion and JavaScript failure keep every step fully visible and line drawn", async ({ page, browser }) => {
  ...
  await page.screenshot({ path: `artifacts/motion-stage-9/regression/screenshots/reduced-${page.viewportSize()!.width}.png`, scale: "css" });
  const context = await browser.newContext({ javaScriptEnabled: false, viewport: page.viewportSize()! });
  const staticPage = await context.newPage(); await staticPage.goto("http://127.0.0.1:3109/"); await assertFinal(staticPage);
