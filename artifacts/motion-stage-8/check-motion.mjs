...
try {
  for (const width of [1440, 768, 390, 360]) {
    const context = await browser.newContext({ viewport: { width, height: 900 }, deviceScaleFactor: 2, hasTouch: width < 768 });
    ...
    for (const route of routes) {
      ...
      if ([1440, 390].includes(width) && ['/services/domestic', '/about', '/contact'].includes(route)) {
        ...
        await page.screenshot({ path: `artifacts/motion-stage-8/after/${route.replaceAll('/', '-').slice(1)}-${width}-motion.png`, fullPage: true, scale: 'css' });
      }
  ...
  await browser.close();
  await writeFile(`artifacts/motion-stage-8/${serviceRecheck ? 'service-final-' : ''}motion-results.json`, JSON.stringify({ results, blockedSubmissions }, null, 2));
}
