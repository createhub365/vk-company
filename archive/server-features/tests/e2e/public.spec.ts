...
test("history, resize and repeated route changes preserve one static hero visual", async ({ page }) => {
  ...
  await page.goto("/");
  await expect(page.locator(".cinematic-hero-background")).toHaveCount(1);
  await expect(page.getByRole("img", { name: /courier truck travelling/i })).toBeVisible();
  expect(await page.locator("canvas").count()).toBe(0);
});
...
test("removed public tracking routes return not found", async ({ page }) => {
  await page.goto("/");
  await expect(page.locator('a[href="/track"]')).toHaveCount(0);
  await expect(page.locator("#main-navigation").getByRole("link", { name: "Track", exact: true })).toHaveCount(0);
  expect(await (await page.request.get("/sitemap.xml")).text()).not.toContain("/track");
