...
A production-oriented Next.js application for domestic and international courier enquiries. It joins a premium public website and procedural 3D dispatch experience to an owner-only Supabase operations area for enquiries, quotes, bookings, manual shipments, verified shipment events and support.

## What is implemented

- Public pages: home, domestic, international, quote request, about, contact, FAQ, privacy-review slot and terms-review slot.
...
- A Supabase project, or the Supabase CLI and Docker, for admin and existing database workflows only.
...
Open `http://localhost:3000`. Configure SMTP for Quote and follow [manual FormSubmit activation](CONTACT_FORMSUBMIT.md) for Contact. Supabase is not required for those forms. Preserve any existing `.env.local` and edit only the needed settings. Admin workflows still require Supabase.
...
5. The owner creates a manual shipment from the booking and records verified shipment events for operational use.
